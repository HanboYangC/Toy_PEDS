...
ws[-1]=0.99
all_loss=[]
weight_name_sparse = 'weights_test_sparse'
...
np.save(os.path.join(weight_dir_sparse, 'loss'), all_loss_np)
#%%
'''Search w Dense'''
...
for w in ws_d:
    ...
    all_loss.append(w_loss)
all_loss_np=np.array(all_loss)
np.save(os.path.join(weight_dir_dense, 'loss'), all_loss_np)
#%%
# sparse_loss=np.nanmean(np.load(os.path.join(weight_dir_sparse, weight_name_sparse + '.npy')), axis=1)
# dense_loss=np.nanmean(np.load(os.path.join(weight_dir_dense, weight_name_dense + '.npy')), axis=1)
